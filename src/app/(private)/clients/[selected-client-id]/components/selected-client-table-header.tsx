...
export function SelectedClientTableHeader() {
  const { setRecordingsFilters } = useGeneralContext();
  const [localQuery, setLocalQuery] = useState("");
  const params = useParams();
  const selectedClientId = params["selected-client-id"] as string;
  ...
  return (
    <div className="flex w-full flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <AudioRecorder
        ...
        customIcon={Plus}
        buttonClassName="bg-gradient-to-r from-primary to-primary px-4 py-2.5 rounded-lg text-sm font-semibold text-white shadow-lg shadow-primary/25 transition-all hover:shadow-primary/40 active:scale-95"
      />
    </div>
  );
}
