...
function getComponentTable(component: AIComponent): TableContent | null {
  ...
  if (Array.isArray(d.appointments) && d.appointments.length > 0) {
    ...
    return {
      head: ["Data", "Horário", "Tipo", "Advogado", "Observações"],
      body: arr.map((a) => [  
        a.date ?? "-",
        a.time ?? "-",
        a.type ?? "-",
...
/**
 * Gera o PDF do prontuário médico a partir dos dados estruturados.
 * Mesma estratégia do overview: jsPDF + autoTable, sem html2canvas.
 */
